...
def check_date_format(date):
    ''' Return true if input parametere matches below date format. '''

    x = re.search(
        r'(0[1-9]|[12]\d|3[01])\s(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s([12]\d{3})', date)
    if x:
        return True
    x = re.search(
        r'(0[1-9]|[12]\d|3[01])/(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\/([12]\d{3})', date)
    if x:
        return True
